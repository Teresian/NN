...
.lll_tab_btnCloseContainer { width: 40px; margin: 0 0 0 auto; cursor: pointer; display: flex; }
.lll_tab_btnClose { border-radius: 10px; background: var(--button-close); transition: background-color 0.15s ease; border: none; box-shadow: 0 0 1px black; height: 18px; width: 18px; padding: 1px 0 0 0.5px; margin: auto; font-weight: 550; color: #2f3451; transition: background-color 0.1s ease-out; font-size: 20px; line-height: 0; cursor: pointer; user-select: none; }
...
.lll_popup_container { width: 100%; height: 100%; color: rgb(231, 231, 231); display: flex; flex-direction: column; gap: 12px; }
.lll_popup_title { font-size: 16px; font-weight: 500; color: rgb(231, 231, 231); text-align: center; }
...
.lll_input_number { padding: 5px 10px 5px 5px; margin: auto 0; border: 1px solid #ced4da; border-radius: 5px; }
...
(function () {
    ...
    let Config = {
        market: {
            api: {
                mwi: 'https://www.milkywayidle.com/game_data/marketplace.json',
                milkyapi: 'https://raw.githubusercontent.com/holychikenz/MWIApi/main/milkyapi.json',
                custom: '',
            },
    ...
    const UiLocale = {
        ...
        battleDrop: {
            ...
            overview: {
                ...
                income: { zh: '总计价值', en: 'Current income' },
                dailyIncome: { zh: '每天收入', en: 'Daily income' },
                luck: { zh: '当前运气', en: 'Current luck' },
                incomeExpt: { zh: '期望产值', en: 'E[current income]' },
    ...
    const Ui = new class {
        ...
        numberInput(options, uiOptions = null) {
            let input = Ui.elem('input', 'lll_input_number');
            this.applyOptions(input, uiOptions);
            input.type = 'number';
    ...
    class TabbedPopup extends Popup {
        ...
        construct() {
            ...
            this.pageTitles = [];
            const closeBtn = Ui.div('lll_tab_btnCloseContainer', Ui.elem('button', 'lll_tab_btnClose', '×'));
            closeBtn.onclick = () => { this.close(); };
            this.root = Ui.div('lll_popup_root', [
                Ui.div({ style: 'display: flex;' }, [this.btnContainer, closeBtn]),
                this.pageContainer
            ]);
    ...
    class PlainPopup extends Popup {
        ...
        construct() {
            this.root = Ui.div({ style: 'z-index: 200; position: fixed; top: 0; left: 0; height: 100%; width: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center;' }, [
                Ui.div({ style: 'height: 100%; width: 100%; background-color: var(--color-midnight-800); opacity: .8;', onclick: () => { this.close(); } }),
                Ui.div({ style: 'margin: -1px -2px -2px -2px; border: 1.5px solid rgba(214, 222, 255, 0.3); border-radius: 8px; background-color: var(--tab-background); display: flex; flex-direction: column; min-height: 0; position: absolute; min-width: 300px; max-width: 98%; min-height: 100px; max-height: 98%; padding: 10px; box-shadow: 0 0 4px 4px hsla(0, 0.00%, 81.60%, 0.28); font-size: 14px; font-weight: 400; overflow: auto;' },
                    Ui.div('lll_popup_container', [
                        Ui.div('lll_popup_title', this.title),
                        this.contentGen(),
                    ])
                )
            ]);
    ...
    const CharacterData = new class {
        ...
        updateLevel(skills) {
            ...
            const { stamina, intelligence, defense, attack, power, ranged, magic } = this.skillLevel;
            this.skillLevel.combat = (stamina + intelligence + defense + Math.max(attack + power, ranged * 2, magic * 2)) / 5;
        }
    };
    ...
    const Market = new class {
        ...
        update(afterUpdated = null) {
            ...
            if (source.type !== 'custom') source.addr = Config.market.api[source.type];
        ...
        #initSpecialItemPrices() {
            this.specialItemPrices = {
                ...
                '/items/cowbell': {
                    ...
                    bid: this.getPriceFromAPI('/items/bag_of_10_cowbells', 'bid') / 10,
                },
                '/items/chimerical_quiver': {
                    ask: this.getPriceFromAPI('/items/mirror_of_protection', 'ask'),
                    bid: this.getPriceFromAPI('/items/mirror_of_protection', 'bid'),
                },
                '/items/sinister_cape': {
                    ask: this.getPriceFromAPI('/items/mirror_of_protection', 'ask'),
                    bid: this.getPriceFromAPI('/items/mirror_of_protection', 'bid'),
            ...
            for (let iter = 0; iter < maxIter; ++iter) {
                for (let [boxHrid, items] of Object.entries(clientData.openableLootDropMap)) {
                    ...
                    if (this.chestCosts[boxHrid]) {
                        const { keyAsk, keyBid, entryAsk, entryBid } = this.chestCosts[boxHrid];
         ...
         * @returns {number}
         */
        getPriceFromAPI(itemHrid, priceType = 'bid', enhanceLevel = 0) {
         ...
         * @returns {number}
         */
        getPriceByHrid(itemHrid, priceType = 'bid', enhanceLevel = 0) {
            if (!this.marketData?.market) return null;
            const marketPrice = this.getPriceFromAPI(itemHrid, priceType, enhanceLevel);
            if (marketPrice) return marketPrice;
            if (priceType === 'ask') return this.getPriceByHrid(itemHrid, 'bid', enhanceLevel) / 0.98;
    ...
    const BattleData = new class {
        ...
        onNewBattle(msg) {
            ...
            for (let player of msg.players) {
                ...
                Object.values(player.totalLootMap).forEach(loot => {
                    ...
                    let bidPrice = Market.getPriceByName(itemName) || 0;
            ...
            for (let mapHrid in this.mapData) {
                ...
                for (const [_, drops] of Object.entries(dropData.bossDrops)) {
                    for (const item of drops) {
                        ...
                        itemCount[item.hrid] += DropAnalyzer.itemCountExpt(item);
                    }
                }
                ...
                for (const [hrid, drops] of Object.entries(dropData.monsterDrops)) {
                    ...
                    for (const item of drops) {
                        itemCount[item.hrid] ??= 0;
                        itemCount[item.hrid] += cnt * DropAnalyzer.itemCountExpt(item);
                    }
                }
        ...
        getCurrentDropData(playerName = null) {
            ...
            return this.getDropData(this.currentMapHrid, this.runCount, playerName);
        }
    };
    ...
    const BattleHistory = new class {
        ...
        update() {
            ...
            const /** @type {BattleHistoryDataEntry} */ current = {
                ...
                income: stat.current.income,
                profit: stat.current.profit,
                luck: stat.luck,
    ...
    const BattleDropAnalyzer = new class {
        ...
        getItemRarity(itemHrid) {
            const value = Market.getPriceByHrid(itemHrid);
        ...
        analyzeCurrent(playerName = null) {
            ...
            return {
                ...
                daily: {
                    ...
                    /** @type {number} */ incomeExpectation: dailyIncomeExpectation,
                    /** @type {number} */ incomeVariance: dailyIncomeVariance,
                    /** @type {number} */ profitExpectation: dailyProfitExpectation,
                },
    ...
    const BattleDropAnalyzerUi = new class {
        ...
        constructor() {
            // 在加载完分赃按钮后添加统计按钮
            observeNode(".Button_battlePlayerLoot__custom", "body", () => this.addButton());
        ...
        constructOverviewPanel() {
            ...
            const itemText = (hrid, rarity, count) => {
                ...
                let itemDiv = Ui.div('lll_div_item', `
            ...
            const getPlayerDiv = (player) => {
                ...
                const isBeatAvg = stat.current.income > stat.current.incomeExpectation;
                const colorLuck = `color: ${luckColor(stat.luck)}`;
                const colorAvg = `color: ${luckColor(isBeatAvg)}`;
                ...
                const calculateTeamStats = () => {
                    ...
                    // 收集所有玩家的收入和支出数据
                    for (let p of BattleData.playerList) {
                        const playerStat = BattleDropAnalyzer.analyzeCurrent(p);
                        const playerIncome = playerStat.current.income;
                        // 支出 = 收入 - 利润，即食物等消耗的成本
                        const playerExpense = playerIncome - playerStat.current.profit;

                        teamStats[p] = {
                            income: playerIncome,
                            expense: Math.max(0, playerExpense) // 确保支出不为负数
                        };
                        totalIncome += playerIncome;
                        totalExpense += Math.max(0, playerExpense);
                    }

                    return { teamStats, totalIncome, totalExpense };
                };

                const { teamStats, totalIncome, totalExpense } = calculateTeamStats();
                const currentPlayerStats = teamStats[player];

                // 计算收入占比
                const incomeRatio = totalIncome > 0 ? (currentPlayerStats.income / totalIncome) * 100 : 0;

                // 计算收入占比颜色 (从红到绿的渐变)
                const getIncomeRatioColor = (ratio) => {
                    // 根据队伍人数计算平均值作为中间值
                    const averageRatio = 100 / BattleData.playerList.length;
                    // 将百分比映射到0-1之间，以平均值为中间点
                    const normalizedRatio = Math.min(ratio / (averageRatio * 2), 1);
                    const hue = normalizedRatio * 120; // 120度是绿色，0度是红色
                    return Utils.HSVtoRGB(hue / 360, 0.8, 0.9).rgb;
                };

                // 分析收入占比情况并给出称号
                const analyzePlayerRole = () => {
                    if (BattleData.playerList.length === 1) {
                        return "独行侠";
                    }

                    const currentIncomeRatio = incomeRatio;

                    // 计算其他队友的收入占比
                    const otherPlayersRatios = BattleData.playerList
                        .filter(p => p !== player)
                        .map(p => totalIncome > 0 ? (teamStats[p].income / totalIncome) * 100 : 0);

                    // 计算其他队友的平均收入占比
                    const otherPlayersAvgRatio = otherPlayersRatios.length > 0
                        ? otherPlayersRatios.reduce((sum, ratio) => sum + ratio, 0) / otherPlayersRatios.length
                        : 0;

                    // 找出最高收入占比
                    const allRatios = BattleData.playerList.map(p => totalIncome > 0 ? (teamStats[p].income / totalIncome) * 100 : 0);
                    const maxIncomeRatio = Math.max(...allRatios);

                    // 调试信息
                    console.log(`玩家 ${player}: 收入占比${currentIncomeRatio.toFixed(1)}%`);
                    console.log(`其他队友平均占比: ${otherPlayersAvgRatio.toFixed(1)}%`);
                    console.log(`最高收入占比: ${maxIncomeRatio.toFixed(1)}%`);

                    // 固定规则（优先级最高）
                    if (currentIncomeRatio > 80) {
                        return "吸血狂魔";
                    } else if (currentIncomeRatio > 70) {
                        return "万磁王";
                    } else if (currentIncomeRatio < 10) {
                        return "超级黑奴";
                    }

                    // 相对于其他队友平均值的规则
                    if (otherPlayersAvgRatio > 0) {
                        if (currentIncomeRatio >= otherPlayersAvgRatio * 2) {
                            return "狗王";
                        } else if (currentIncomeRatio >= otherPlayersAvgRatio * 1.5) {
                            return "大狗";
                        } else if (currentIncomeRatio >= otherPlayersAvgRatio * 1.25) {
                            return "小狗";
                        } else if (currentIncomeRatio >= otherPlayersAvgRatio * 1.1) {
                            return "略欧";
                        }
                    }

                    // 相对于最高收入占比的规则
                    if (maxIncomeRatio > 0 && currentIncomeRatio !== maxIncomeRatio) {
                        const ratioToMax = currentIncomeRatio / maxIncomeRatio;
                        if (ratioToMax < 0.3) {
                            return "黑奴";
                        } else if (ratioToMax < 0.5) {
                            return "非酋";
                        } else if (ratioToMax < 0.7) {
                            return "脸黑";
                        }
                    }
                ...
                const playerRole = analyzePlayerRole();
                const incomeRatioColor = `color: ${getIncomeRatioColor(incomeRatio)}`;
                ...
                const table = Ui.elem('table', { style: 'font-weight: bold; line-height: 1.2; width: 100%;' }, `
                    <tr style="${colorLuck}">
                        <td style="text-align: right;">${UiLocale.battleDrop.overview.income[language]}:</td>
                        ${tablePrice(Utils.formatPrice(stat.current.income))}
                    </tr>
                    <tr style="${colorLuck}">
                        <td style="text-align: right;">${UiLocale.battleDrop.overview.dailyIncome[language]}:</td>
                        ${tablePrice(`${Utils.formatPrice(stat.daily.income)}/d`)}
                    </tr>
                    <tr style="${colorLuck}">
                        <td style="text-align: right;">${UiLocale.battleDrop.overview.luck[language]}:</td>
                        ${tablePrice(`${(stat.luck * 100).toFixed(2)}%`)}
                    </tr>
                    <tr style="${incomeRatioColor}">
                        <td style="text-align: right;">收入占比:</td>
                        ${tablePrice(`${incomeRatio.toFixed(1)}%`)}
                    </tr>
                    <tr style="color: #ffd700;">
                        <td style="text-align: right;">收入分析:</td>
                        <td style="text-align: right; font-weight: bold;">${playerRole}</td>
                        <td></td>
                    </tr>
                    <tr><td colspan="3"><div class="lll_separator" style="margin: 6px -3px"></div></td></tr>
                    <tr style="${colorAvg}">
                        <td style="text-align: right;">${UiLocale.battleDrop.overview.incomeExpt[language]}:</td>
                        ${tablePrice(Utils.formatPrice(stat.current.incomeExpectation))}
                    </tr>
                    <tr style="${colorAvg}">
                        <td style="text-align: right;"><span class="lll_text_noSelect" style="color: var(--border)">└</span>${UiLocale.battleDrop.overview.stdDev[language]}:</td>
                        ${tablePrice(Utils.formatPrice(Math.sqrt(stat.current.incomeVariance)))}
                    </tr>
                    <tr style="${colorAvg}">
                        <td style="text-align: right;">${UiLocale.battleDrop.overview.dailyIncomeExpt[language]}:</td>
                        ${tablePrice(`${Utils.formatPrice(stat.daily.incomeExpectation)}/d`)}
                    </tr>
                    <tr style="${colorAvg}">
                        <td style="text-align: right;"><span class="lll_text_noSelect" style="color: var(--border)">└</span>${UiLocale.battleDrop.overview.stdDev[language]}:</td>
                        ${tablePrice(Utils.formatPrice(Math.sqrt(stat.daily.incomeVariance)))}
                    </tr>
                    <tr style="${colorAvg}">
                        <td style="text-align: right;">${UiLocale.battleDrop.overview.dailyProfitExpt[language]}:</td>
                        ${tablePrice(
                    stat.isSteady ? `${Utils.formatPrice(stat.daily.profitExpectation, { precision: stat.daily.profitExpectation < 0 ? 3 : 4 })}/d` :
                        `<span style="margin: 0 1px 0 0;">≥</span>${Utils.formatPrice(stat.daily.profitExpectation, { precision: stat.isSteady ? 4 : 3 })
                            .replace('-', '<span style="font-family: Consolas, monaco, monospace;">-</span>')
                        }/d`
                )}
                    </tr>
                    <tr><td colspan="3"><div class="lll_separator" style="margin: 6px -2px"></div></td></tr>
                    <tr style="color: #ffc107">
                        <td style="text-align: right;">${xpName}${UiLocale.battleDrop.overview.experience[language]}:</td>
                        ${tablePrice(`${xpPerHour}/h`)}
                    </tr>
                `);
                ...
                if (itemCount === 0) {
                    ...
                    if (runCount >= 1000) {
                        let info = `${UiLocale.battleDrop.overview.info[language](runCount)}!`;
                        let text = Ui.div({
                            style: {
                                textAlign: 'center',
        ...
        constructDetailsPanel() {
            ...
            const customPanel = () => {
                ...
                // 返回到详细页面
                const customButton = Ui.button(UiLocale.battleDrop.distribution.return[language]);
                customButton.onclick = () => {
                    panel.removeChild(contentDiv);
                    detailsPanel();
                };
        ...
        constructSettingsPanel() {
            let panel = Ui.div({ className: 'lll_div_column', style: 'font-size: 15px; margin: 20px; gap: 20px;' });
            ...
            panel.appendChild(SettingsUi.settingRow(UiLocale.battleDrop.settings.sortOrder[language], null, itemSortOrderSelect));

            panel.appendChild(SettingsUi.settingRow(
                UiLocale.battleDrop.settings.displayLimit[language], null, Ui.numberInput({
            ...
            panel.appendChild(SettingsUi.settingRow(
                UiLocale.battleDrop.settings.showNormal[language], null, Ui.checkBox({
            ...
            panel.appendChild(SettingsUi.settingRow(
                UiLocale.battleDrop.settings.sampleRate[language],
                UiLocale.battleDrop.settings.sampleRateDesc[language],
                Ui.slider({
            ...
            panel.appendChild(SettingsUi.settingRow(
                UiLocale.battleDrop.settings.interpolationCount[language],
                UiLocale.battleDrop.settings.interpolationCountDesc[language],
                Ui.slider({
            ...
            let updateMarketInfo = Ui.div('lll_label', `${UiLocale.battleDrop.settings.lastUpdated[language]}: ${new Date(Market.marketData.time * 1000).toLocaleString()}`);
            let updateMarketBtn = Ui.button(UiLocale.battleDrop.settings.updateMarket[language]);
            updateMarketBtn.onclick = () => {
                updateMarketInfo.style.minWidth = getComputedStyle(updateMarketInfo).width;
                updateMarketInfo.innerHTML = `${UiLocale.battleDrop.settings.updating[language]}...`;
                Market.update(() => {
                    updateMarketInfo.innerHTML = `${UiLocale.battleDrop.settings.updateFinish[language]}: ${Utils.formatDate(new Date(Market.marketData.time * 1000))}`;
                });
            };
            panel.appendChild(Ui.div('lll_div_row', [updateMarketBtn, updateMarketInfo]));
        ...
        addButton() {
            ...
            button.textContent = UiLocale.battleDrop.btnLabel[language];
    ...
    const TaskAnalyzerUi = new class {
        ...
        addButton() {
            ...
            let desc = `任务溢出时间: ${Utils.formatDate(overflowDate)}<br>`;
            const rewards = TaskAnalyzer.getTaskExpectedRewards(CharacterData.skillLevel, TaskData.blockedTypes);
            desc += `任务期望奖励: ${Utils.formatPrice(rewards.price)} (${Utils.formatPrice(rewards.coin)} 金币, ${rewards.taskToken.toFixed(2)} 任务代币)<br>`;
            desc += `期望次数:<br>`;
            mapRunCount.forEach(([id, cnt]) => {
                desc += `图 ${id}: ${cnt.total} 次 (剩 ${cnt.rest} 次)<br>`;
            });
            dbg(desc);

            Tooltip.attach(button, Tooltip.description("任务统计", desc));
    ...
    const ChestDropAnalyzerUi = new class {
        ...
        constructOpenChestPopup(openedItem, gainedItems) {
            ...
            const colorLuck = `color: ${luckColor(stat.luck)}`;
            const colorAvg = `color: ${luckColor(stat.income > stat.incomeExpectation)}`;
            const tablePrice = (x) => {
                let i = x.length - 1;
                for (; i >= 0; --i) if (x[i] >= '0' && x[i] <= '9') break;
                const unit = x.slice(i + 1);
            ...
            if (!chestOpenHistory) historyDiv = Ui.div(null, '需安装食用工具');
                ...
                const colorLuckHist = `color: ${luckColor(historyStat.luck)}`;
                const colorAvgHist = `color: ${luckColor(historyStat.income > historyStat.incomeExpectation)}`;
    ...
    function LOCAL() {
        // const cf = DropAnalyzer.test(1.5, 8.6, 0.8);

        // let start = new Date().getTime();

        // for (let i = 0; i < 30; ++i) cf(1048576, 0.123);
        // out(cf(1048576, 0.123)[10]);
        ...
        // init_character_data
        MessageHandler.handleMessageRecv(JSON.stringify(_initCharacterData));

        BattleDropAnalyzerUi.addButton()
        // BattleDropAnalyzerUi.showPopup();

        TaskAnalyzerUi.addButton();

        // dbg(BattleData.getDropData("/actions/combat/chimerical_den", 3, "Weierstras"));
        // dbg(BattleData.getDropData("/actions/combat/golem_cave_elite", 11, "Weierstras"));
        // dbg(BattleData.getDropData("/actions/combat/panda"));

        setTimeout(() => {
            // new_battle
            MessageHandler.handleMessageRecv(JSON.stringify(_newBattle));

            ChestDropAnalyzerUi.showOpenChestPopup({
                "openedItem": {
                ...
                "gainedItems": [
                    ...
                    {
                        "itemHrid": "/items/shield_bash",
                        "count": 1
                    },
                    {
                        "itemHrid": "/items/crippling_slash",
                        "count": 2
                    }
                ]
            });
